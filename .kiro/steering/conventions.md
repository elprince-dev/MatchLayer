...
- **Backend Pydantic models** are the source of truth for API request/response shapes.
- **OpenAPI spec** is auto-generated by FastAPI. Committed (or generated in CI) so the frontend always builds against a known version.
- **Frontend TS types** generated from OpenAPI via `openapi-typescript`. Re-exported via curated names from `packages/shared-types/index.ts` to avoid ugly path types in app code.
- **Frontend Zod schemas** also generated from OpenAPI via `openapi-zod-client`, used for both runtime API-response parsing and form validation. Server-side Pydantic remains authoritative for security; client-side Zod is for UX + early-failure detection of contract drift.
- **Codegen runs in CI** on every PR; a drift check fails the build if committed schemas don't match the live OpenAPI output.
