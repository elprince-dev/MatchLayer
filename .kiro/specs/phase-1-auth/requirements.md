...
4. THE Authenticated_Shell SHALL be implemented as a Next.js Server Component that calls the API to verify the access token (using the refresh cookie when the access token is missing or expired), rendering the children when the verification succeeds and redirecting to `/login?next=<original_path>` via Next.js's `redirect()` when it does not.
