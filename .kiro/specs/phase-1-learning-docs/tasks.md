...
3. Author the Phase 1 Topic_Docs in eight thematic batches (Foundation and tooling → Frontend → Backend → Security → Database and storage → Containerization → Contracts and codegen → Hosting and deploy), running the validator clean after each batch.
...
- [ ] 1. Bootstrap the Learning_Docs_Library skeleton and entry-point files
  - [ ] 1.1 Create the on-disk folder structure for the library
  ...
  - [ ] 1.2 Author `docs/learning/README.md` (Library_Index)
  ...
  - [ ] 1.3 Author `docs/learning/CONVENTIONS.md` (Conventions_Doc)
  ...
  - [ ] 1.4 Author the initial `docs/learning/phase-1/README.md` (Phase_1_Index)
    ...
    - Add the eight thematic-section H2s in this exact order: `Foundation and tooling`, `Frontend`, `Backend`, `Security`, `Database and storage`, `Containerization`, `Contracts and codegen`, `Hosting and deploy`. Leave their bodies empty for now (entries are added per Topic_Doc as it is authored).
    - Add a `Recommended reading order` H2 with an empty ordered list to be filled in as Topic_Docs are added.
    - _Requirements: 2.2, 2.3, 2.4, 2.8, 4.10_

- [ ] 2. Implement the compliance validator (`tools/learning_docs_check.py`)
  - [ ] 2.1 Add the validator dev-dependency setup
  ...
  - [ ] 2.2 Implement the data models and Markdown parser
  ...
  - [ ]\* 2.3 Write smoke tests on the parser and library skeleton
  ...
  - [ ] 2.4 Implement filename and H1 rules (`LDC001`, `LDC002`)
  ...
  - [ ]\* 2.5 Write property test for filename conformance
    - **Property 10: Topic_Doc filename conformance**
    - **Validates: Requirements 2.9, 8.1**

  - [ ]\* 2.6 Write property test for Topic_Doc H1 conformance
    - **Property 33: Topic_Doc H1 conformance**
    - **Validates: Requirements 8.2**

  - [ ] 2.7 Implement the Doc_Template H2-sequence rule (`LDC003`)
  ...
  - [ ]\* 2.8 Write property test for Doc_Template H2 sequence equality
    - **Property 13: Doc_Template H2 sequence equality**
    - **Validates: Requirements 3.1, 3.10, 3.11, 8.3, 8.4, 8.5, 8.7**

  - [ ] 2.9 Implement the per-section content rules (`LDC004`, plus content checks for Introduction, Mental model, MatchLayer Phase 1 usage, Common pitfalls, External reading)
  ...
  - [ ]\* 2.10 Write property test for Introduction learning outcomes
    - **Property 14: Introduction declares at least three learning outcomes**
    - **Validates: Requirements 3.2**

  - [ ]\* 2.11 Write property test for Mental model handhold
    - **Property 15: Mental model section contains a concrete handhold**
    - **Validates: Requirements 3.4, 5.4**

  - [ ]\* 2.12 Write property test that "How it works" is implementation-agnostic
    - **Property 16: How it works section is implementation-agnostic**
    - **Validates: Requirements 3.5**

  - [ ]\* 2.13 Write property test that MatchLayer Phase 1 usage anchors to an Implementation_File
    - **Property 17: MatchLayer Phase 1 usage section contains anchored content**
    - **Validates: Requirements 3.6**

  - [ ]\* 2.14 Write property test for the Common pitfalls labelled-entry rule
    - **Property 18: Common pitfalls section has three labelled entries**
    - **Validates: Requirements 3.7**

  - [ ]\* 2.15 Write property test for External reading section size bounds
    - **Property 19: External reading section size bounds**
    - **Validates: Requirements 3.8, 7.6**

  - [ ] 2.16 Implement file-reference and code-snippet rules (`LDC005`, `LDC006`, `LDC007`, `LDC008`)
  ...
  - [ ]\* 2.17 Write property test that Implementation_File references are well-formed and resolve
    - **Property 27: Implementation_File reference is well-formed and resolves**
    - **Validates: Requirements 6.1, 6.4**

  - [ ]\* 2.18 Write property test that sourced fenced blocks match their source
    - **Property 28: Sourced fenced block matches its source and carries a citation**
    - **Validates: Requirements 6.2, 6.7, 3.12**

  - [ ]\* 2.19 Write property test for the simplified-for-illustration block format
    - **Property 29: Simplified-for-illustration block is labelled and linked**
    - **Validates: Requirements 6.3**

  - [ ]\* 2.20 Write property test for allowed code-block language tags
    - **Property 30: Fenced code blocks use allowed language tags**
    - **Validates: Requirements 6.6**

  - [ ] 2.21 Implement link-integrity rules (`LDC009`, `LDC010`, `LDC011`)
  ...
  - [ ]\* 2.22 Write property test for internal hyperlink resolution
    - **Property 11: Internal hyperlink resolution**
    - **Validates: Requirements 2.10, 7.1, 7.2, 10.7**

  - [ ]\* 2.23 Write property test for internal heading-anchor matching
    - **Property 12: Internal heading-anchor matches a real heading**
    - **Validates: Requirements 7.3**

  - [ ]\* 2.24 Write property test for the https-only external-link scheme
    - **Property 31: External hyperlinks use the https scheme**
    - **Validates: Requirements 7.4**

  - [ ]\* 2.25 Write property test for authoritative-source host preference
    - **Property 32: Authoritative source preference is honored**
    - **Validates: Requirements 7.5**

  - [ ] 2.26 Implement beginner-accessibility rules (`LDC012`, `LDC013`, `LDC014`, `LDC015`)
  ...
  - [ ]\* 2.27 Write property test for domain-term first-use definitions
    - **Property 22: Domain-specific term defined on first use**
    - **Validates: Requirements 5.2**

  - [ ]\* 2.28 Write property test for acronym expanded-form first use
    - **Property 23: Acronym introduced in expanded form on first use**
    - **Validates: Requirements 5.3**

  - [ ]\* 2.29 Write property test for absence of banned phrases
    - **Property 24: Banned phrases are absent**
    - **Validates: Requirements 5.5**

  - [ ]\* 2.30 Write property test for prerequisites declared in Introduction
    - **Property 25: Prerequisites declared in the Introduction**
    - **Validates: Requirements 5.6**

  - [ ]\* 2.31 Write property test for cross-reference handling
    - **Property 26: Cross-reference is hyperlinked or inline-defined**
    - **Validates: Requirements 5.7**

  - [ ] 2.32 Implement coverage and Library_Index rules (`LDC016`, `LDC017`, `LDC018`, `LDC019`, `LDC020`)
    ...
    - _Requirements: 1.4, 1.7, 2.5, 2.6, 2.10, 4.1, 4.10, 4.12, 10.3, 10.7_

  - [ ]\* 2.33 Write property test for the coverage table → Topic_Doc mapping
    - **Property 20: Coverage table maps every entry to an existing Topic_Doc**
    - **Validates: Requirements 4.1, 4.10, 4.12**

  - [ ]\* 2.34 Write property test that consolidated Topic_Docs name every consolidated entry
    - **Property 21: Consolidated Topic_Docs name every consolidated entry**
    - **Validates: Requirements 4.11**

  - [ ]\* 2.35 Write property test for the Library_Index Phase Sub-Libraries listing
    - **Property 1: Phase Sub-Libraries listing reflects on-disk state**
    - **Validates: Requirements 1.4**

  - [ ]\* 2.36 Write property test for Phase-1 non-interference under sibling addition
    - **Property 2: Phase-1 non-interference under sibling addition**
    - **Validates: Requirements 1.6**

  - [ ]\* 2.37 Write property test for the External Sources app-README listing
    - **Property 3: External Sources reflects every present app README**
    - **Validates: Requirements 1.7**

  - [ ]\* 2.38 Write property test for the no-50-word-duplication rule
    - **Property 4: No 50-word duplication from upstream sources**
    - **Validates: Requirements 1.8**

  - [ ]\* 2.39 Write property test for the Phase_1_Index introduction word count and reference
    - **Property 5: Phase_1_Index introduction word count and reference**
    - **Validates: Requirements 2.3**

  - [ ]\* 2.40 Write property test for the Phase_1_Index thematic-section sequence
    - **Property 6: Phase_1_Index thematic-section sequence equality**
    - **Validates: Requirements 2.4**

  - [ ]\* 2.41 Write property test that every Topic_Doc is listed exactly once
    - **Property 7: Every Topic_Doc listed exactly once across thematic sections**
    - **Validates: Requirements 2.5**

  - [ ]\* 2.42 Write property test for Topic_Doc entry markup integrity
    - **Property 8: Topic_Doc entry markup integrity in the Phase_1_Index**
    - **Validates: Requirements 2.6, 2.7**

  - [ ]\* 2.43 Write property test for the Recommended reading order endpoints
    - **Property 9: Recommended reading order is exhaustive with foundation-first and hosting-last endpoints**
    - **Validates: Requirements 2.8**

  - [ ]\* 2.44 Write property test for the Library_Index Non-goals minimum bullets
    - **Property 34: Library_Index Non-goals has at least four bullets**
    - **Validates: Requirements 10.1**

  - [ ]\* 2.45 Write property test that the Phase_1_Sub_Library does not over-discuss future phases
    - **Property 35: Phase_1_Sub_Library does not over-discuss future phases**
    - **Validates: Requirements 10.6**

  - [ ] 2.46 Implement the CLI entry point and reporter
...
- [ ] 3. Checkpoint — validator is complete and clean against the empty library
...
- [ ] 5. Author the "Frontend" Topic_Docs
  ...
  - [ ] 5.6 Author `framer-motion-and-reduced-motion.md`
  ...
  - [ ] 5.7 Author `next-themes-and-system-default.md`
    - Cover `next-themes` and the system-default theme.
    - _Requirements: 4.1, 4.3_

  - [ ] 5.8 Author `nextjs-proxy-security-headers.md`
...
- [ ] 6. Author the "Backend" Topic_Docs
  ...
  - [ ] 6.5 Author `connection-pooling-and-pre-ping.md`
    - Cover connection pooling and the role of `pool_pre_ping`. May be consolidated into 6.4 per Req 4.11; if consolidated, name both coverage entries verbatim in the Topic_Doc's `Introduction` and update the `Topic coverage` rows to share the same filename.
    - _Requirements: 4.1, 4.4, 4.11_

  - [ ] 6.6 Author `alembic-migrations-and-empty-baseline.md`
    - Cover Alembic migrations and the empty baseline strategy, anchoring to `apps/api/alembic/`.
    - _Requirements: 4.1, 4.4_

  - [ ] 6.7 Author `structlog-and-json-logging.md`
  ...
  - [ ] 6.8 Author `request-id-middleware.md`
...
- [ ] 7. Checkpoint — backend and frontend Topic_Docs are complete
...
- [ ] 8. Author the "Security" Topic_Docs
  ...
  - [ ] 8.1 Author `security-headers-explained.md`
  ...
  - [ ] 8.3 Author `structured-logging-as-pii-defense.md`
  ...
  - [ ] 8.4 Author `secrets-management.md`
  ...
  - [ ] 8.5 Author `dependency-and-supply-chain-scanning.md`
  ...
  - [ ] 8.6 Author `threat-model-categories.md`
...
- [ ] 9. Author the "Database and storage" Topic_Docs
  ...
  - [ ] 9.2 Author `postgres-vs-minio.md`
  ...
  - [ ] 9.3 Author `redis-fundamentals.md`
    ...
    - _Requirements: 4.1, 4.6, 10.6_

  - [ ] 9.4 Author `named-docker-volumes.md`
  ...
  - [ ] 9.5 Author `pgvector-and-the-phase-2-boundary.md`
...
- [ ] 10. Author the "Containerization" Topic_Docs
  ...
  - [ ] 10.5 Author `production-dockerfiles.md`
    - Cover the production Dockerfiles in `infra/docker/` and what each instruction does, anchoring to both Dockerfiles. May consolidate with 10.3 or 10.4 per Req 4.11.
    - _Requirements: 4.1, 4.7, 4.11_

  - [ ] 10.6 Author `distroless-and-runtime-hardening.md`
...
- [ ] 12. Author the "Hosting and deploy" Topic_Docs
  ...
  - [ ] 12.3 Author `dependabot-configuration.md`
    - Cover Dependabot configuration, anchoring to `.github/dependabot.yml`.
    - _Requirements: 4.1, 4.9_

  - [ ] 12.4 Author `branch-protection-and-required-checks.md`
  ...
  - [ ] 12.5 Author `vercel-hobby-tier-hosting.md`
  ...
  - [ ] 12.6 Author `flyio-backend-hosting.md`
  ...
  - [ ] 12.7 Author `aws-s3-in-phase-1.md`
    - Cover AWS S3 as the Phase 1 file-storage backend.
    - _Requirements: 4.1, 4.9_

  - [ ] 12.8 Author `aws-migration-path-preservation.md`
    ...
    - _Requirements: 4.1, 4.9, 10.6_

- [ ] 13. Finalize the Phase_1_Index and library cross-links
  - [ ] 13.1 Complete the `Topic coverage` table
    - Fill the `Topic_Doc filename` cell for every coverage-list row, using the design's mapping as the baseline. Where consolidation has been applied (e.g., 6.4 absorbed 6.5), the consolidated rows share the same filename and the absorbing Topic_Doc names every consolidated entry verbatim in its `Introduction` (Req 4.11).
    - _Requirements: 4.1, 4.10, 4.11, 4.12_

  - [ ] 13.2 Populate the eight thematic sections in the Phase_1_Index
  ...
  - [ ] 13.3 Populate `Recommended reading order`
    - Build the numbered list as the union of every Topic_Doc filename present under `docs/learning/phase-1/`. The first entry is from `Foundation and tooling`; the last entry is from `Hosting and deploy`. Order entries within each thematic block to build conceptually (foundation → application surface → hardening → deploy).
    - _Requirements: 2.8_

  - [ ] 13.4 Update the Library_Index for the populated state
...
- [ ] 14. Final checkpoint — full validator run on the populated library
...
```json
{
  "waves": [
    ...
    { "id": 13, "tasks": ["2.17", "13.1", "13.4"] },
    { "id": 14, "tasks": ["2.18", "13.2"] },
    { "id": 15, "tasks": ["2.19", "13.3"] },
    { "id": 16, "tasks": ["2.20"] },
    { "id": 17, "tasks": ["2.22"] },
    { "id": 18, "tasks": ["2.23"] },
    { "id": 19, "tasks": ["2.24"] },
